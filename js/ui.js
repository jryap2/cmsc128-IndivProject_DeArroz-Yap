...
function renderInboxTasks() {
  ...
  filteredTasks.forEach(task => {
    // Note: We use the task object itself to find the index in the store
    // This handles cases where filtering changes the visual index
...
async function onRevertTask(index) {
  if (index >= 0 && index < store.completed_tasks.length) {
    // REFACTOR: Await the store action, no saveTasks()
    await store.restoreTask(index); 
    // Note: Original code logic for restoreTask in store might need check if it handles completed->inbox
    // Assuming you updated store.restoreTask to handle logic. 
    // If store.restoreTask is for deleted, we might need a specific revert function.
    // For now, based on your previous store structure, let's use addTask.
    // Ideally: store.updateTaskStatus(id, 'inbox')
    
    // Since we are refactoring, let's assume store.restoreTask works for deleted, 
    // and for completed we need to move it back.
    // Let's use the generic update if we can, or manual move.
    
    // Simpler approach compatible with the new store:
    const task = store.completed_tasks[index];
    if (task) {
        await store.updateTask(task._id, { status: 'inbox' }); // This relies on my previous api instruction
        // Manually move in store for UI if store.updateTask doesn't auto-move arrays (it usually doesn't to avoid complexity)
        // Actually, let's force a reload or handle array move manually in UI/Store. 
        // Best approach: Reload or let store handle array splice.
        // As per the previous store code I gave you:
        store.completed_tasks.splice(index, 1);
...
function renderCompletedTasks() {
  ...
  filteredTasks.forEach(task => {
    ...
    if (!selectionMode) {
      taskDiv.querySelector('.revert-button')?.addEventListener('click', () => {
         // Manual revert logic here since we don't have a direct 'revert' in store for completed->inbox
         const taskToRevert = store.completed_tasks[actualIndex];
         store.completed_tasks.splice(actualIndex, 1);
         taskToRevert.status = 'inbox';
         store.tasks.unshift(taskToRevert);
         store.updateTask(actualIndex, { status: 'inbox' }); // Sync
         render();
      });
...
// REFACTOR: Removed saveTasks() calls
...
export async function onSaveTask() {
  ...
  // 1. PERFORM ASYNCHRONOUS ACTION (Create or Update)
  if (editIndex !== undefined) {
    // Note: store.updateTask is async now
    await store.updateTask(parseInt(editIndex, 10), task);
  } else {
    // Note: store.addTask is async now
    await store.addTask(task);
  }

  // 2. ONLY RENDER AND CLEAN UP *AFTER* THE NETWORK CALL IS COMPLETE
  render(); // Rerender the list with the confirmed task
  ...
  switch (currentFilter.type) {
    ...
    case 'sort':
      if (currentFilter.value === 'newest') {
        filtered = filtered.reverse(); // Simplified reverse for now
      }
      break;
  }
  return filtered;
...
function addSelectionToolbarListeners(toolbar) {
  ...
  toolbar.querySelector('.revert-selected')?.addEventListener('click', async () => {
    const indices = Array.from(selectedTasks).sort((a, b) => b - a);
    for (const index of indices) {
      // Revert logic for selection
      const task = store.completed_tasks[index];
      if (task) {
          store.completed_tasks.splice(index, 1);
          task.status = 'inbox';
          store.tasks.unshift(task);
          await store.updateTask(task._id, { status: 'inbox' });
      }
    }
    // await store.saveTasks(); <-- REMOVED
    exitSelectionMode();
